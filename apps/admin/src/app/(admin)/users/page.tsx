...
export default async function UsersPage({
  ...
  return (
    <div className="p-6">
      ...
      <form method="GET" action="/users" className="flex gap-2 mb-4">
        <input
          type="search"
          name="q"
          defaultValue={q}
          placeholder="Search by name or email…"
          className="w-72 px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500"
        />
        <button
          type="submit"
        ...
        {q && (
          <Link href="/users" className="px-3 py-2 text-sm text-slate-500 hover:text-slate-700 transition-colors">
            Clear
          </Link>
        )}
      </form>
            ...
            {profiles?.map(profile => (
              <tr key={profile.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3 text-slate-600">
                  {profile.job_title ?? <span className="text-slate-300">—</span>}
                </td>
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                href={`/users?${new URLSearchParams({ ...(q ? { q } : {}), page: String(page - 1) })}`}
                className="px-3 py-1.5 border border-slate-200 rounded-md hover:bg-slate-50 text-slate-600 transition-colors"
              >
                ← Previous
              </Link>
            )}
